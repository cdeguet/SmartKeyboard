...
JNIEXPORT jlong JNICALL Java_jp_co_omronsoft_openwnn_OpenWnnDictionaryImplJni_createWnnWork
  (JNIEnv *env, jobject obj, jobject fileDescriptor, jlong start, jlong length)
{
	NJ_JNIWORK*		work;
	...
	if( work != NULL ) {
    	...
    	jint fd = (*env)->GetIntField(env, fileDescriptor, sDescriptorField);

    	work->dicBuffer = malloc(length);
    	if (work->dicBuffer == NULL) {
     	   free(work);
      	   return 0;
    	}
    	...
    	offset = 0;
    	for (i=0; i<7; i++) {
    		jp_dic_data[i] = (NJ_UINT8*)(work->dicBuffer + offset);
    		offset += jp_dic_size[i];
    	}
    	jp_con_data[0] = (NJ_UINT8*)(work->dicBuffer + offset);

		for( i = 0 ; i < NJ_MAX_DIC ; i++ ) {
            work->dicHandle[ i ]    = jp_dic_data[ i ];
			work->dicSize[ i ]      = jp_dic_size[ i ];
			work->dicType[ i ]      = jp_dic_type[ i ];
		}

        /* Set the rule dictionary if the rule data exist */
        work->dicSet.rHandle[ NJ_MODE_TYPE_HENKAN ] = jp_con_data[ 0 ];
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    sDescriptorField = (*env)->GetFieldID(env, clazz, "descriptor", "I");
